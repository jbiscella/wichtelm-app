...
- **Price · primary** — the primary-TF chart over the window `[entry − max(P × 1.5, 30) bars, exit + 10 bars]` where `P` is the largest indicator period referenced by the entry / exit Scenarios on the primary timeframe. Frame header lists the chart contents (HA candles + main-pane overlays + RSI sub-pane when applicable). The chart image itself is produced by heerwisch-jfreechart with main-pane indicators added via `addIndicator(...)`; the entry bar is annotated with `Annotation.EntryExitMarker(direction = LONG_ENTRY / SHORT_ENTRY, glyphStyle = UP_TRIANGLE)`, the exit bar (when present) with `EntryExitMarker(direction = LONG_EXIT / SHORT_EXIT, glyphStyle = DOWN_TRIANGLE` for Scenario-driven exits / `ARROW_DOWN` for forced closes — stop-loss, take-profit, end-of-series). The held interval is shaded with `Annotation.TimeRangeHighlight(fillColor, opacity = 0.15)` where the fill is chosen by **trade outcome** — `LONG_POSITION` (green) for winning closed trades, `SHORT_POSITION` (red) for losing closed trades, `NEUTRAL` (grey) for the still-open position — matching the TradingView Strategy Tester convention; the entry/exit marker colors above remain direction-based (industry standard). An RSI sub-pane SVG (Y pinned to 0–100, semantic-coloured dashed threshold lines, danger-zone shading at alpha 0.16, vertical reference lines at entry and exit) is appended below the heerwisch image when a Background series declares `rsi(...)`. The frame's footer shows `▲ entry <ts> · in position · Mh · exit <ts> ▼` (or the open-trade variant `mark <ts> · window end`).
