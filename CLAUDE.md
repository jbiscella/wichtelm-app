...
`wichtelm-app` is an end-user **backtesting application** for trading strategies on single financial instruments using historical OHLC data, built around **Heikin-Ashi (HA)** candles as its visual and analytical backbone: every report chart renders HA candles and the DSL exposes first-class HA pattern primitives alongside the standard RSI / MACD / moving-average / pivot families (HA is the backbone, not an exclusive constraint).
...
The application is built on top of `ha-track` (Heikin-Ashi-track) libraries published to Maven Central under the `net.jacopobiscella` namespace.
...
The `demo/` directory ships runnable examples designed to cover the whole DSL feature surface: **five "clean" strategies, one per timeframe** (weekly `1w`, daily `1d`, 4-hour `4h`, hourly `1h`, the daily one multi-timeframe) cover most of it, and **two daily "showcase" strategies** (deliberately not profit-tuned) sweep up every remaining catalog primitive — seven in total. Demo market data is **synthetic fixtures** under deliberately fake instrument names (never real tickers) so it can never be mistaken for real prices, generated by `demo/GenerateData.java` (deterministic, JDK-only — it lives under `demo/`, outside `src/`, so it is never compiled into the app). Two instruments model their asset class realistically: **`TSTX`** is a synthetic equity at `1d`/`1w` with **weekday-only** bars (realistic stock calendar; daily/weekly charts show only the normal small weekend gaps), and **`TSTC`** is a synthetic crypto asset at `1h`/`4h` that trades **24/7** (continuous bars are realistic for crypto, and keep the intraday charts gap-free). Each price path runs a bull → bear → bull regime cycle so strategies see both wins and losses.

Each demo runs two ways, both through the `wichtelm` CLI with no bespoke tooling:

- `data_source = "csv"` — offline, against the committed `demo/data/TSTX_*.csv` and `demo/data/TSTC_*.csv` fixtures. Zero network, zero credentials. The committed `demo/reports/tstx-*-report.html` and `tstc-*-report.html` are these CSV runs and are the stable reference reports.
