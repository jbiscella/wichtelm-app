...
| **P11** | (spec rule) entries *may* append stop/take/trailing after `Then` | no distinct diagnostic — a non-protective step after `Then` is **reported as P10** with `only 'And with stop_loss at' / 'And with take_profit at' / 'And with trailing_stop at' may follow Then` | Only `And with stop_loss at` / `And with take_profit at` / `And with trailing_stop at` may follow `Then`. |
